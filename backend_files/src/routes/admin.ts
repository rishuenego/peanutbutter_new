...
import { isAdmin, AdminRequest, generateAdminToken, removeAdminToken, validateAdminToken } from '../middleware/adminAuth.js'

const router = Router()

// Safe JSON parse helper
function safeJsonParse(value: unknown, fallback: unknown[] | Record<string, unknown> = []): unknown {
...
router.post('/products', isAdmin, async (req: AdminRequest, res) => {
  try {
    const {
...
router.put('/products/:id', isAdmin, async (req: AdminRequest, res) => {
  try {
    const { id } = req.params
    const {
      name, slug, description, shortDescription, category, texture,
...
router.patch('/coupons/:id/toggle', isAdmin, async (req, res) => {
  try {
    ...
    // Get current status
    const coupon = await getOne<{ is_active: number }>('SELECT is_active FROM coupons WHERE id = ?', [id])
    if (!coupon) {
