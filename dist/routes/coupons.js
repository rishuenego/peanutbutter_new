...
router.post('/validate', async (req, res) => {
    try {
        ...
        const coupon = await getOne('SELECT * FROM coupons WHERE code = ? AND is_active = 1', [code.toUpperCase()]);
        if (!coupon) {
        ...
        const discountAmount = Math.min((orderAmount * coupon.discount_percentage) / 100, coupon.max_discount_amount);
        res.json({
            success: true,
            coupon: {
                code: coupon.code,
