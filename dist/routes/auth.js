...
router.post('/register', async (req, res) => {
    try {
        ...
        const result = await execute(`INSERT INTO users (email, name, password, phone, created_at, updated_at)
       VALUES (?, ?, ?, ?, NOW(), NOW())`, [email, fullName, hashedPassword, phone || null]);
        const newUser = await getOne('SELECT * FROM users WHERE id = ?', [
            result.insertId,
        ]);
        // Log in the user
...
router.put('/profile', isAuthenticated, async (req, res) => {
    ...
    try {
        await execute(`UPDATE users SET phone = ?, address = ?, city = ?, state = ?, pincode = ?, updated_at = NOW()
       WHERE id = ?`, [phone, address, city, state, pincode, userId]);
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, salt);
        // Update password
        await execute('UPDATE users SET password = ?, updated_at = NOW() WHERE email = ?', [hashedPassword, email]);
        // Delete OTP
        otpStore.delete(email);
        res.json({
            success: true,
...
router.post('/set-password', isAuthenticated, async (req, res) => {
    try {
        ...
        await execute('UPDATE users SET password = ?, updated_at = NOW() WHERE id = ?', [hashedPassword, userId]);
        res.json({
            success: true,
